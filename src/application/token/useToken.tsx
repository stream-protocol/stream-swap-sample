...
export const useToken = create<TokenStore>((set, get) => ({
    ...
    set((s) => ({
      ...
      tokenListSettings: {
        ...
        [USER_ADDED_TOKEN_LIST_NAME]: {
          ...
          mints: (s.tokenListSettings[USER_ADDED_TOKEN_LIST_NAME].mints ?? new Set()).add(
  ...
  sortTokens(tokens: SplToken[]) {
    ...
    // noQuantumSOL
    const whiteListMints = whiteList
      .filter((token) => !isQuantumSOL(token))
